...
class CNN1DNetV2_RevGrad(nn.Module):

    def __init__(self, num_classes=109, num_channels=32):
        super(CNN1DNetV2_RevGrad, self).__init__()

        # Feature extractor part (convolutional layers)
        self.feature_extractor = nn.Sequential(
            nn.Conv1d(num_channels, 128, 3, stride=2, padding=1),
            nn.BatchNorm1d(128), nn.ReLU(),
            nn.Conv1d(128, 256, 3, stride=2, padding=1), nn.BatchNorm1d(256),
            nn.ReLU(), nn.Conv1d(256, 512, 3, stride=2, padding=1),
            nn.BatchNorm1d(512), nn.ReLU())

        # Classifier part (fully connected layers)
        self.classifier = nn.Linear(64001, num_classes)
